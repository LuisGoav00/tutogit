	...
	<body>
		...
		<?php if( isset($row) ) : ?>
			...
			<table class="table">
				...
				<tbody>
                    <?php
                      foreach($query as $columnas) {          
                       /* echo '<tr>';
						echo '<th scope="row">'. $columnas['id'].'</th>';
						echo '<td>'.$columnas['nombre'].'</td>';
						echo '<td>'.$columnas['marca'].'</td>';
						echo '<td>'.$columnas['modelo'].'</td>';
						echo '<td>'.$columnas['precio'].'</td>';
						echo '<td>'.$columnas['unidades'].'</td>';
						echo '<td>'.utf8_encode($columnas['detalles']).'</td>';
						echo '<td><img src='.$columnas['imagen'].'</td>';
					    echo '</tr>';¨*/
                        ?>
                        <tr>
						...
						<td><input type="button" value="Editar Objeto" onClick="obtenerDatos()"/></td>
					</tr>
                        <?php } ?>
